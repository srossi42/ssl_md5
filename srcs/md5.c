...
uint32_t	ft_md5_rotate_left(uint32_t x, uint32_t n)
{
    return ((x << n) | (x >> (32 - n)));
}

uint32_t ft_md5_reverse_bits(uint32_t value){
...
void ft_md5_init_abcd(t_md5_struct *md5_struct){
    MD5_A = MD5_H0;
    MD5_B = MD5_H1;
    MD5_C = MD5_H2;
    MD5_D = MD5_H3;
...
void ft_md5_update_abcd(t_md5_struct *md5_struct, md5_byte_t word[4], int j){
    int         tmp;
    uint32_t    w_g;
    ...
    MD5_D = MD5_C;
    MD5_C = MD5_B;
    MD5_B += ft_md5_rotate_left(MD5_A + MD5_F + g_md5_k[j] + w_g, g_md5_r[j]);
    MD5_A = tmp;
}

void ft_md5_update_hx(t_md5_struct *md5_struct){
    MD5_H0 += MD5_A;
    MD5_H1 += MD5_B;
    MD5_H2 += MD5_C;
    MD5_H3 += MD5_D;
...
void ft_md5_get_fg(t_md5_struct *md5_struct, int j){
    if (j < 16) {
...
void ft_md5_reverse_words(t_md5_struct *md5_struct){
    MD5_H0 = ft_md5_reverse_bits(MD5_H0);
    MD5_H1 = ft_md5_reverse_bits(MD5_H1);
    MD5_H2 = ft_md5_reverse_bits(MD5_H2);
    MD5_H3 = ft_md5_reverse_bits(MD5_H3);
}
